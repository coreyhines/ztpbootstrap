...
def validate_dhcp_option(option: Dict) -> Tuple[bool, Optional[str]]:
    ...
    if code in PROTECTED_DHCP_OPTIONS:
        return False, f"DHCP option {code} ({PROTECTED_DHCP_OPTIONS[code]}) is protected and cannot be customized"
...
def validate_dhcp_config(dhcp_config: Dict) -> Tuple[bool, Optional[str]]:
    ...
    if "ipv4" in dhcp_config:
        ...
        if "dns" in ipv4:
            is_valid, error = validate_dns_servers(ipv4["dns"])
            if not is_valid:
    ...
    if "ipv6" in dhcp_config:
        ...
        if "dns" in ipv6:
            ...
            is_valid, error = validate_dns_servers(ipv6["dns"])
            if not is_valid:

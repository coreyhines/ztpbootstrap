...
prompt_with_default() {
    ...
    if [[ "${NON_INTERACTIVE:-false}" == "true" ]]; then
        ...
        if [[ -n "$env_value" ]]; then
            ...
            value="$default_value"
            # Always ensure ENROLLMENT_TOKEN has a value (default to "arista" for testing)
            if [[ "$var_name" == "ENROLLMENT_TOKEN" ]]; then
...
load_existing_installation_values() {
    ...
    if [[ -n "$config_file" ]] && [[ -f "$config_file" ]]; then
        ...
        log "  Loaded values from config.yaml (enrollment_token: ${EXISTING_ENROLLMENT_TOKEN:+set})"
...
interactive_config() {
    # In non-interactive mode, ensure ENROLLMENT_TOKEN is read from environment if set
    # Otherwise ensure ENROLLMENT_TOKEN is always set for testing (default to "arista")
    # Use ${ENROLLMENT_TOKEN:-} to avoid unbound variable error with set -u
    if [[ "${NON_INTERACTIVE:-false}" == "true" ]]; then
        # In non-interactive mode, use environment variable if set, otherwise use existing or default
        if [[ -z "${ENROLLMENT_TOKEN:-}" ]]; then
            if [[ -n "${EXISTING_ENROLLMENT_TOKEN:-}" ]]; then
                export ENROLLMENT_TOKEN="${EXISTING_ENROLLMENT_TOKEN}"
            else
...
PYTHON_VERIFY
    ...
    # In upgrade mode, use existing password hash if available (unless --reset-pass was used)
        ...
        if [[ "${NON_INTERACTIVE:-false}" == "true" ]]; then
            if [[ -n "${ADMIN_PASSWORD_HASH:-}" ]]; then
                ...
                # Use default password "admin" in non-interactive mode
                log "Non-interactive: Using default password 'admin' for Web UI."
                ADMIN_PASSWORD="admin"
                # Hash the password using Python (same method as interactive mode)
                ADMIN_PASSWORD_HASH=$(python3 -c "from werkzeug.security import generate_password_hash; print(generate_password_hash('admin'))" 2>/dev/null || python3 -c "import hashlib, base64; print('pbkdf2:sha256:' + base64.b64encode(hashlib.pbkdf2_hmac('sha256', b'admin', b'ztpbootstrap', 100000)).decode())" 2>/dev/null || echo "")
                if [[ -z "$ADMIN_PASSWORD_HASH" ]]; then
                    error "Failed to hash default password. Authentication will not be configured."
                    exit 1
                fi
                SET_ADMIN_PASSWORD="true"
            fi
    ...
    # For enrollment token, always default to "arista" for testing (any string works)
    # Use environment variable if set, otherwise existing value, otherwise "arista"
    prompt_with_default "Enrollment token (from CVaaS Device Registration)" "${ENROLLMENT_TOKEN:-${EXISTING_ENROLLMENT_TOKEN:-arista}}" ENROLLMENT_TOKEN "true"
...
generate_yaml_config() {
    # Safety net: ensure ENROLLMENT_TOKEN is always set (default to "arista" for testing)
    ...
    ENROLLMENT_TOKEN="${ENROLLMENT_TOKEN:-arista}"
    ...
    # ENROLLMENT_TOKEN is always set by safety net above, so no need to validate

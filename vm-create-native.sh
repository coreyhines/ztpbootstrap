    ...
    if [[ "$DOWNLOAD_TYPE" == "cloud" ]]; then
        ...
        case "$distro" in
            fedora|Fedora)
                ...
                patterns=("Fedora-Cloud-Base-*.${arch}.qcow2" "Fedora-Cloud-Base-*.${arch}.qcow2.xz")
                ;;
            ubuntu|Ubuntu)
                if [[ "$arch" == "aarch64" ]] || [[ "$arch" == "arm64" ]]; then
                    patterns=("ubuntu-*-server-cloudimg-arm64.img")
                else
            ...
            for pattern in "${patterns[@]}"; do
                ...
                if [[ -n "$found_file" ]] && [[ -f "$found_file" ]]; then
                    ...
                    if [[ "$found_file" == *.xz ]]; then
                        ...
                        if [[ -f "$extracted" ]]; then
                            log_info "Using extracted version: $extracted"
                            ISO_PATH="$extracted"
                            return 0
                        else

...
VERSION="${VERSION:-43}"
TEST_MATRIX="${TEST_MATRIX:-$SCRIPT_DIR/test-matrix.yaml}"
MAX_ITERATIONS="${MAX_ITERATIONS:-1}"
KEEP_ON_FAILURE="${KEEP_ON_FAILURE:-false}"
REPORT_DIR="${REPORT_DIR:-./tests/test-reports}"
...
Options:
    ...
    --report-dir DIR        Directory for test reports (default: ./tests/test-reports)
...
create_vm() {
    ...
    "$VM_CREATE_SCRIPT" --download "$DISTRO" --type cloud --arch aarch64 --version "$VERSION" --headless --name "$VM_NAME" > "$create_log" 2>&1 &
    local vm_pid=$!
...
" 2>/dev/null || true
            ...
            ssh -o ConnectTimeout=5 -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -p "$SSH_PORT" "${SSH_USER}@localhost" \
                "cd ${ztpbootstrap_dir} && \
                yq e '.cvaas.enroll_chars = \"test_token_for_automated_testing\"' -i config.yaml 2>/dev/null || true && \

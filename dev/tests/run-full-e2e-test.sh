...
rm -f ztpbootstrap-test*.qcow2 2>/dev/null || true
"$VM_CREATE_SCRIPT" --download "$DISTRO" --type cloud --arch aarch64 --version "$VERSION" --headless 2>&1 | tee /tmp/e2e-vm-create.log &
VM_PID=$!
...
if [ -f "$WAIT_SSH_SCRIPT" ]; then
    ...
    "$WAIT_SSH_SCRIPT" localhost 2222 "$DEFAULT_USER" 600 2 || {

...
VM Creation Arguments:
    Arguments after '--' are passed directly to dev/scripts/vm-create-native.sh.
...
For more VM creation options, see: dev/scripts/vm-create-native.sh --help
...
if [[ "$SKIP_VM_CREATE" == "true" ]]; then
    ...
    if ps aux | grep -i "qemu-system-aarch64" | grep -v grep > /dev/null; then
        ...
        "$VM_CREATE_SCRIPT" "${VM_BASE_ARGS[@]}" 2>&1 | tee /tmp/test-vm-create.log &
        VM_PID=$!
...
if [[ "$RESTORE_BACKUP" == "true" ]]; then
    ...
    echo "  2. ✓ Repository cloned and updated"
    echo ""
    echo "Next steps - Test fresh installation:"
    echo "  1. SSH into the VM:"
    echo "     ssh -p 2222 ${DEFAULT_USER}@localhost"
    echo ""
    echo "  2. Run interactive setup:"
    ...
    echo "     ./setup-interactive.sh"
    echo ""
    echo "  3. The interactive setup will guide you through:"
    echo "     - Configuration of all paths and variables"

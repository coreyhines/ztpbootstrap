    ...
    def runCommands(self, cmdList):
        ...
        try:
            ...
            cmdOutput = subprocess.check_output(
                "echo -e '" + cmds + "' | " + self.fastCliBinary,
                shell=True,
        ...
        if cmdOutput:
            for line in cmdOutput.split("\n"):
                if line.startswith("%"):
                    err = cmdOutput
                    log(
                        "Error running commands: [{cmdStr}], err: {err}".format(
...
def configureAndRestartNTP(ntpServer):
    ...
    configureNtpCmds = [
        "en",
        "configure",
        "ntp server {ntpServer} prefer iburst".format(ntpServer=ntpServer),
        "exit",
    ]
...
class BootstrapManager(object):
    ...
    def getBootstrapURL(self, addr):
        ...
        if not (addr.startswith("//") or addr.startswith("http://") or addr.startswith("https://")):
            addr = "//" + addr
    ...
    def checkWithRedirector(self):
        ...
        try:
            ...
            response = requests.post(
                self.redirectorURL.geturl(), data=payload, headers=headers, proxies=proxies
            )
    ...
    def getCertificatePaths(self):
        ...
        try:
            ...
            log(
                "Failed to get the path of the stored client certs, err: {err}".format(err=e.output)
            )
    ...
    def getBootstrapScript(self):
        ...
        try:
            tpmStatus = self.pathHelper.getEntity(
                "cell/{cellID}/hardware/tpm/status".format(cellID=self.cellID)
            )
        ...
        headers["X-Arista-SoftwareVersion"] = getKeyValueFromFile("/etc/swi-version", "SWI_VERSION")
        ...
        log(
            "Step 3.1 done, bootstrap script fetched and stored at {bootScriptPath}".format(
